...
## 🔧 Correctifs du 08/09 (avant restitution du 09/09)

Un audit du repo a montré que l'appli Streamlit (`dq-compass-app-en/`) ne
démarrait plus du tout, et que le catalogue officiel avait été remplacé par
l'exemple de portabilité BIS. Correctifs apportés :

- **Modules manquants recréés** : `data_quality_checks.py` (checks basiques +
  détection RGPD), `default_rules_catalog.py` (catalogue de règles par défaut
  affiché en haut de la page Define Rules + tips de rédaction), `pdf_report.py`
  (export PDF via reportlab). Ces 3 fichiers n'avaient jamais été commités,
  ce qui faisait planter les 3 pages de l'appli.
- **Catalogue officiel restauré** : `catalogue/control_catalogue.csv` contient
  à nouveau les 6 règles DQ01-DQ06 (BF-CAT-03). L'exemple BIS (portabilité)
  a été déplacé dans `examples/bis-domain/`, sur le modèle de `examples/hr-domain/`.
- **Point d'entrée unifié** : suppression de `app.py` (doublon obsolète),
  `run.sh`/`run.bat` lancent désormais `Home.py`.
- **`engine.py` factorisé** : la logique d'exécution (`execute_run()`) est
  partagée entre le CLI et `engine_wrapper.py` (appli Streamlit), pour
  garantir le même evidence pack quel que soit le point d'entrée.
- **Evidence pack persistant depuis l'appli** : un run déclenché depuis
  Streamlit écrit désormais un vrai `runs/<run_id>/` (manifest.json,
  results_summary.csv, exceptions/, engine.log) dans `dq-compass-engine/runs/`
  au lieu du dossier temporaire de l'OS — et un bouton "Download evidence pack
  (ZIP)" a été ajouté sur la page Quality Report (BF-AUD-01/02).
- **Contrôle RGPD intégré comme une erreur** : en plus de l'alerte affichée à
  l'upload, le check RGPD est maintenant exécuté comme une règle de plus
  (`RGPD-01`) dans `results_summary.csv` — il remonte dans le rapport, les
  exceptions et la boucle de validation, comme demandé en session de travail.
- **Scorecard (BF-REP-01)** et **vue de couverture** (BF-REP-03) ajoutées sur
  la page Quality Report.
- **Paramètre lignes/colonnes** ajouté sur la page Upload Data (restreindre
  l'analyse à un sous-ensemble du CSV).
- **Import de règles** : ajout du format Excel (.xlsx) et d'une sélection
  ligne par ligne des règles à garder avant import (au lieu d'un import
  tout-ou-rien).
- `.gitignore` ajouté, `__pycache__` retiré du suivi git.

Tout est validé par des tests automatisés (compilation + exécution headless
de chaque page via `streamlit.testing.v1.AppTest`, run du moteur en CLI et
via l'appli, reproductibilité vérifiée) avant ce commit.
...
### Application Streamlit (nouveau dossier `dq-compass-app-en/`)

| Fichier | Description | Lignes |
|---------|-------------|--------|
| `app.py` | Page d'accueil Streamlit | ~110 |
| `pages/1_Upload_Data.py` | Étape 1 : Upload et validation CSV | ~200 |
| `pages/2_Define_Rules.py` | Étape 2 : Formulaire de règles | ~280 |
| `pages/3_Quality_Report.py` | Étape 3 : Rapport visuel (KPIs, graphiques, exceptions) | ~340 |
...
│
├── dq-compass-app-en/            ✨ NOUVEAU DOSSIER (frontend)
│   ├── app.py
│   ├── pages/
│   │   ├── 1_Upload_Data.py
│   │   ├── 2_Define_Rules.py
