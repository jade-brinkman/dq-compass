...
du catalogue est exécuté par une des fonctions génériques de rules.py.
...
La logique d'exécution est factorisée dans `execute_run()` : c'est cette même
fonction que `main()` (CLI) et l'application Streamlit (`engine_wrapper.py`)
appellent, pour garantir qu'un run déclenché depuis l'appli produit exactement
le même evidence pack (manifest + results_summary + exceptions + log) qu'un
run en ligne de commande.
...
def build_manifest(run_id, timestamp, data_dir, valid_rows, catalogue_path, run_dir, log, filters=None):
    ...
    catalogue_copy_path = run_dir / catalogue_copy_name
    # engine_wrapper.py (l'appli Streamlit) écrit parfois déjà le catalogue
    # temporaire de la session directement à cet emplacement : dans ce cas
    # catalogue_path == catalogue_copy_path, et il ne faut pas se copier
    # soi-même (shutil.copyfile lève SameFileError).
    if catalogue_path.resolve() != catalogue_copy_path.resolve():
        shutil.copyfile(catalogue_path, catalogue_copy_path)
    ...
    manifest = {
        ...
        "catalogue_sha256": sha256_of_file(catalogue_copy_path),
        "filters": filters or {},
    }
...
def execute_run(catalogue_path: Path, data_dir: Path, run_dir: Path, run_id: str, timestamp: str,
                 log: logging.Logger, dimensions: str = "", rule_ids: str = "", extra_results=None) -> dict:
    """
    Exécute un run complet (chargement catalogue -> exécution -> écriture de
    l'evidence pack) et retourne un dict {"results", "summary_df", "exceptions",
    "manifest"}.

    Utilisée à la fois par `main()` (CLI) et par `engine_wrapper.py` (appli
    Streamlit) : un run déclenché depuis l'appli produit exactement le même
    `runs/<run_id>/` (manifest.json + results_summary.csv + exceptions/ +
    engine.log) qu'un run en ligne de commande — c'est ce qui garantit un
    evidence pack audit-ready quel que soit le point d'entrée (BF-AUD-01/02).

    `extra_results` permet d'ajouter des lignes de résultat déjà calculées en
    dehors du catalogue (ex. le contrôle RGPD auto de l'appli Streamlit) sans
    toucher au contrat catalogue <-> moteur : chaque élément doit être un dict
    au même format que ce que retourne `run_rule()`.
    """
    ...
    catalogue = filter_catalogue(catalogue, dimensions, rule_ids, log)
    if catalogue.empty:
        raise EngineError("Aucune règle ne correspond aux filtres --dimensions/--rules fournis.")
    ...
    # --- results_summary.csv ---
    summary_df = pd.DataFrame([{k: r[k] for k in RESULTS_COLUMNS} for r in results])
    ...
    manifest = build_manifest(
        ...
        filters={"dimensions": dimensions or None, "rules": rule_ids or None},
    )

    return {"results": results, "summary_df": summary_df, "exceptions": exceptions, "manifest": manifest}
...
def main():
    ...
    parser.add_argument(
        "--dimensions", default="",
        help="Sous-ensemble de dimensions à exécuter/mettre en avant, ex. 'Complétude,Réconciliation' "
    ...
    try:
        outcome = execute_run(
            catalogue_path, data_dir, run_dir, run_id, timestamp, log,
            dimensions=args.dimensions, rule_ids=args.rules,
        )

        # --- copies "dernière exécution" pour le Reporting Layer ---
        latest_dir = Path(args.latest_dir)
        shutil.copyfile(run_dir / "results_summary.csv", latest_dir / "results_summary.csv")
        latest_exc_dir = latest_dir / "exceptions"
        if latest_exc_dir.exists():
            shutil.rmtree(latest_exc_dir)
        shutil.copytree(run_dir / "exceptions", latest_exc_dir)
