...
métier n'est codée en dur : les 6 `logic_type` obligatoires du cahier des
besoins (BF-CAT-02) sont implémentés par 6 fonctions génériques dans
`engine/rules.py`, appliquées dynamiquement à n'importe quel jeu de règles
respectant le format du catalogue. Un 7e `logic_type`, `unique_composite`
(unicité sur une combinaison de colonnes), a été ajouté comme extension
documentée pour les datasets multi-colonnes (voir `examples/`) — à valider
avec l'équipe avant restitution si on veut rester strictement sur les 6
valeurs du cahier des besoins.

## Structure

```
catalogue/control_catalogue.csv   Catalogue officiel (6 règles DQ01-DQ06) — fichier séparé du code
...
engine/rules.py                   Les 7 fonctions génériques (logic_type — 6 obligatoires + 1 extension)
engine/engine.py                  Orchestrateur (chargement, exécution, sorties) — execute_run() est
                                   la fonction réutilisée à la fois par le CLI et par l'appli Streamlit
...
examples/hr-domain/               Preuve de portabilité — jeu de données RH, catalogue dédié
examples/bis-domain/              Preuve de portabilité — jeu de données réel BIS (77 992 lignes),
                                   catalogue dédié (voir dq-compass-engine-notes.md pour le détail)
```

L'appli Streamlit (`dq-compass-app-en/`) appelle ce moteur via
`engine_wrapper.py`, qui utilise `execute_run()` et écrit son evidence pack
au même endroit (`runs/<run_id>/`) qu'un run lancé en CLI — un run déclenché
depuis l'appli est donc tout aussi auditable qu'un run en ligne de commande.
